...
$(document).ready(function() {	
	...
	$(".jumbotron").trigger("click");// .->classe, #->id
    _lblErrore.hide();
	$("#btnLogin").on("click", controllaLogin)
	// il submit deve partire anche senza click 
	// ma con il solo tasto INVIO
	$(document).on('keydown', function(event) {	
	   if (event.keyCode == 13)//Controllo se è stato cliccato il tasto invia  
		   controllaLogin();
	});
	function controllaLogin(){
        _username.removeClass("is-invalid");  // bordo rosso textbox errore
		_username.prev().removeClass("icona-rossa");  // rimuove icona rossa			
        ...
        if (_username.val() == "") {
			...
			let user=_username.val();//Prendo il valore dell'user
			// md5 restituisce una word esadecimale, quindi è obbligatorio .toString()
			let pass=CryptoJS.MD5(_password.val()).toString();//Cripto la password
	...
	_lblErrore.children("button").on("click", function(){
		_lblErrore.hide(); //Chiusura errore
	})
});